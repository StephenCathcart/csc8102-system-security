...
import static uk.co.stephencathcart.system.security.Dictionary.Stage.COMPLETE;
import static uk.co.stephencathcart.system.security.Dictionary.Stage.STAGE_FIVE;
import static uk.co.stephencathcart.system.security.Dictionary.Stage.STAGE_FOUR;
import static uk.co.stephencathcart.system.security.Dictionary.Stage.STAGE_ONE;
import static uk.co.stephencathcart.system.security.Dictionary.Stage.STAGE_THREE;
import static uk.co.stephencathcart.system.security.Dictionary.Stage.STAGE_TWO;
...
public final class Dictionary {
    ...
    /**
     * Enumeration to state which stage the dictionary is at. The higher stages
     * take longer to compute passwords.
     */
    enum Stage {
        STAGE_ONE,
        STAGE_TWO,
    ...
    /**
     * A cached queue of raw passwords read from file. Avoids the need to read
     * from file multiple times.
     */
    private Queue<String> cachedPasswords;

    /**
     * A queue of permuted passwords. For example, permutations of sophie are
     * sOphie, SOpHIe etc. For optimisation, we calculate all permutations at
     * once then exhaust all further possibilities of each before moving on to
     * the next.
     */
    private Queue<String> passwordPermutations;

    /**
     * Flag to avoid generating the short alphanumerical passwords multiple
     * times.
     */
    ...
    /**
     * Checks if a password is available. If not, it will try to update the list
     * of passwords.
     *
    ...
    /**
     * Updates the queue with new passwords depending the current stage. Each
     * stage gets more complicated and takes longer to generate. It first starts
     * by reading simple lists of passwords from boy and girl names before
     * moving on to randomly generating passwords.
     */
    ...
    /**
     * Load a queue of passwords from file. We also cache these raw passwords so
     * we don't need to read from the same file again.
     *
    ...
    /**
     * Generates a queue of case sensitive passwords each with a four digit
     * number appended. The possible number of combinations for a name is 2^n,
     * where n equals the length of the name. To avoid memory issues, one call batches 10,000 results of a
     * permutation e.g. sOphie1, sOphie2 etc.
     *
     * Modified from a stackexchange post.
     * @see https://codereview.stackexchange.com/questions/3222/finding-all-upper-lower-case-combinations-of-a-word
     */
    ...
    /**
     * Takes a permutation (e.g. sOphIe) and iterates through 0-9999 appending
     * the number to it. Add these to the next password batch.
     */
    ...
    /**
     * Generates three-letter password permutations from the global alphabet. As
     * the alphabet is 71 characters long, we only create three letter passwords
     * given us a complexity of 71^3 rather than 71^4. We add the last letter
     * when creating the password batch.
     *
